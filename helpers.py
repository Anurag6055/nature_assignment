...
def generate_short_code(db: Session) -> str:
    ...
    code = ''.join(random.choice(config.settings.CODE_CHARACTERS))
    for i in range(config.settings.CODE_LENGTH):
        exists = db.query(database.URLItem).filter_by(short_code = code).first()
        if not exists:
            return code

def create_short_url(db: Session, url: str, alias: str = None, expiry: int = None):
    if alias:
        if db.query(database.URLItem).filter_by(short_code=alias).first():
